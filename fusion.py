...
parser.add_argument('cm', metavar='cm', type=str, help='path location for confusion matrix')
parser.add_argument('--weights', metavar='weights', type=float, nargs='+', help='weights for each stream')
parser.add_argument('--usePotion', metavar='potion', help='use potion')
...
# Get the labels
annotation_file = 'data/YMJA/ymja_test_split_1_rawframes.txt' #'data/ucf101/ucf101_val_split_1_rawframes.txt'
labels = []
...
labels = ['No Penalty', 'Tripping', 'Hooking', 'Slashing', 'Holding']
df_cm = pd.DataFrame(matrixConfusion, labels, labels)
plt.figure(figsize=(3,3))
# sn.set(font_scale=1.6) # for label size
g = sn.heatmap(df_cm, annot=True,  cmap='coolwarm') # font size
